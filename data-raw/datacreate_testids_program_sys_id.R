...
# )
testids_program_sys_id <- data.frame(
  program = c(
...
if (anyNA( frs_from_programid(testids_program_sys_id$program,testids_program_sys_id$pgm_sys_id)$lat )) {stop("some of the testids_program_sys_id are not in the FRS database or lack lat,lon")}
## or  
...
x <- frs_from_programid(testids_program_sys_id$program, testids_program_sys_id$pgm_sys_id)[, c("PGM_SYS_ACRNMS", "PRIMARY_NAME")]
